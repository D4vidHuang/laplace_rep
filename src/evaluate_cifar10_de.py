...
def run_evaluation(ood_dataset, batch_size):
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')

    # 1. load 5 MAP
    model_paths = [os.path.join('models', f'CIFAR_de_model_{i + 1}.pt') for i in range(5)]
    models = []
    for p in model_paths:
        m = WideResNet(16, 4, num_classes=10).to(device)
...
if __name__ == '__main__':
    ...
    acc, conf, cls_auroc, ood_acc, ood_conf, ood_auroc = \
        run_evaluation(args.ood, args.batch_size)
