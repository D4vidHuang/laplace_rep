...
class BasicBlockVB(nn.Module):
    def __init__(self, in_planes, out_planes, stride, var0=1, dropRate=0.0):
        super().__init__()
...
class NetworkBlockVB(nn.Module):

    def __init__(self, nb_layers, in_planes, out_planes, block, stride, var0=1, dropRate=0.0):
        super().__init__()
...
class WideResNetVB(nn.Module):

    def __init__(self, depth, widen_factor, num_classes, num_channel=3, var0=1, droprate=0, feature_extractor=False):
        super().__init__()
        ...
        self.block1 = NetworkBlockVB(n, nChannels[0], nChannels[1], block, 1, var0, droprate)
        self.block2 = NetworkBlockVB(n, nChannels[1], nChannels[2], block, 2, var0, droprate)
        self.block3 = NetworkBlockVB(n, nChannels[2], nChannels[3], block, 2, var0, droprate)
        # global average pooling and classifier
        self.bn1 = nn.BatchNorm2d(nChannels[3])
        self.relu = nn.ReLU(inplace=True)
