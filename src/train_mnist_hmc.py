'''Train MNIST with HMC implementation'''
...
import argparse
from resources.csgmcmc.models.mnist_lenet import MNISTLeNet
import numpy as np
import random

parser = argparse.ArgumentParser(description='HMC MNIST Training')
...
parser.add_argument('--temperature', type=float, default=1./60000,
...
net = MNISTLeNet()  # Changed to MNISTLeNet
if use_cuda:
    net.cuda(device_id)
...
for epoch in range(args.epochs):
    ...
    if acc > best_acc:
        ...
        torch.save(state, os.path.join(model_dir, f'mnist_hmc_epoch_{epoch}.pt'))
        best_acc = acc 