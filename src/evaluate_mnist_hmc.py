...
parser = argparse.ArgumentParser(description='HMC MNIST Evaluation')
...
def test_ensemble():
    ...
    model_files = sorted(glob(os.path.join(model_dir, 'mnist_hmc_epoch_*.pt')))
    ...
    results_file = os.path.join(project_root, 'results_mnist_hmc.txt')
